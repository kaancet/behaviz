...
def plot_impact(
    x: ArrayLike, ys: ArrayLike, ax: Optional[BehavizAxes] = None, spec: Optional[PlotSpec] = None, **overrides
    ...
    assert x.shape[0] == ys.shape[1], (
        f"Shape of {spec.x.label}({x.shape[0]}) is not equal to shape of {spec.y.label}({ys.shape[1]})."
    )
